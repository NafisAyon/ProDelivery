...
namespace ProDelivery.Controllers
{
    public class ReceiversController : Controller
    {
        ...
        public ActionResult Edit(string id)
        {
            ...
            ViewBag.Req_Status = new List<SelectListItem>() { new SelectListItem{ Text="Delivering", Value = "Delivering" },
        ...
        public ActionResult SignUp(Receiver receiver)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    ViewBag.ContactTaken = "Phone No has been already taken by another User!";
                    return View();
                }
                else
                {
        ...
        public ActionResult Login(TempUser tempUser)
        {
            if (ModelState.IsValid)
            {
                ...
                {   
                    ...
                    HttpCookie cookie = new HttpCookie("user", "rec_user_email");
                    cookie.Values["rec_user_email"] = user.Rec_Email;
        ...
        public ActionResult UserProfile()
        {
            ...
            string email = Convert.ToString(cookie.Values["rec_user_email"]);
            var user = db.Receivers.Where(u => u.Rec_Email.Equals(email)).FirstOrDefault();
