    ...
    public class SendersController : Controller
    {
        ...
        public ActionResult SignUp(Sender sender)
        {
            if (ModelState.IsValid) {
                 if (db.Senders.Where(u => u.Sender_Email == sender.Sender_Email).Any())
                  ...
                  {
                    ViewBag.ContactTaken = "Phone Number has been already taken by another User!";
                    return View();
                  }
                 else {
        ...
        public ActionResult Login(TempUser2 tempUser2)
        {
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                    Session["sec_user_email"] = user.Sender_Email;
                    HttpCookie cookie = new HttpCookie("user", "sec_user_email");
                    cookie.Values["sec_user_email"] = user.Sender_Email;
        ...
        public ActionResult UserProfile()
        {
            ...
            string email = Convert.ToString(cookie.Values["sec_user_email"]);
        ...
        public ActionResult Services(Consignment consignment)
        {
            if (ModelState.IsValid)
            {
                ...
                return RedirectToAction("Delivered", "Senders");
            }
            return View();
        }
